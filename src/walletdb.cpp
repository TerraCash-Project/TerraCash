...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call TerraCash with fAggressive=true to
    // get as much data as possible.
    // Rewrite terracashd data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> terracashdData;
    bool allOK = dbenv.TerraCash(newFilename, true, terracashdData);
    if (terracashdData.empty()) {
        LogPrintf("TerraCash(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("TerraCash(aggressive) found %u records\n", terracashdData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, terracashdData) {
        if (fOnlyKeys) {
